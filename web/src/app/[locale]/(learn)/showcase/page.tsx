...
import { ChevronRight, Play, MonitorPlay } from "lucide-react";
    ...
    <div className="py-4">
      ...
      <div className="mb-8">
        <div className="mb-3 flex items-center gap-2">
          <MonitorPlay size={20} className="text-[var(--color-text-secondary)]" />
          <h1 className="text-3xl font-bold">{t("title")}</h1>
        </div>
        <p className="text-[var(--color-text-secondary)]">{t("subtitle")}</p>
        <div className="mt-4">
          <Link
            href={`/${locale}/showcase/s01`}
      ...
      {/* Layer groups — mirrors the layers page style */}
        ...
        {LAYERS.map((layer, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="border-t border-zinc-200 bg-zinc-50/50 px-6 py-4 dark:border-zinc-800 dark:bg-zinc-900/50">
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {versionInfos.map(({ id, info, meta }) => (
                    <Link
                      key={id}
                      href={`/${locale}/showcase/${id}`}
                      className="group"
                    >
                      <div className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm transition-all hover:border-zinc-300 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-zinc-700">
                        ...
                        {meta?.keyInsight && (
                          <p className="mt-2 text-xs leading-relaxed text-zinc-500 line-clamp-2 dark:text-zinc-400">
                            {meta.keyInsight}
                          </p>
                        )}
                        {/* Open showcase hint */}
                        <div className="mt-3 flex items-center gap-1 text-xs font-medium text-zinc-400 transition-colors group-hover:text-zinc-700 dark:text-zinc-600 dark:group-hover:text-zinc-300">
                          <Play size={11} />
                          <span>{t("open_showcase")}</span>
                        </div>
                      </div>
                    </Link>
                  ))}
              ...
              {/* Composition arrow between layers */}
