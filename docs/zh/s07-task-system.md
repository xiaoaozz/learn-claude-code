...
class TaskManager:
    def __init__(self, tasks_dir: Path):
        self.dir = tasks_dir
        self.dir.mkdir(exist_ok=True)   # 如不存在则创建 .tasks/ 目录
...
def _clear_dependency(self, completed_id):
    for f in self.dir.glob("task_*.json"):  # 扫描所有任务
        task = json.loads(f.read_text())
        if completed_id in task.get("blockedBy", []):
            task["blockedBy"].remove(completed_id)  # 移除阻塞关系
            self._save(task)
```
...
def update(self, task_id, status=None,
    ...
    if status:
        task["status"] = status
        if status == "completed":           # 传播完成状态到图
            self._clear_dependency(task_id)
    if add_blocked_by:                      # 新增依赖边
        task["blockedBy"] = list(set(task["blockedBy"] + add_blocked_by))
    if remove_blocked_by:                   # 删除依赖边
...
TOOL_HANDLERS = {
    ...
    "task_list":   lambda **kw: TASKS.list_all(),  # LLM 可见完整任务图
