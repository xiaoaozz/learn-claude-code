...
class TaskManager:
    def __init__(self, tasks_dir: Path):
        self.dir = tasks_dir
        self.dir.mkdir(exist_ok=True)   # .tasks/ディレクトリを作成
        self._next_id = self._max_id() + 1

    def create(self, subject, description=""):
        task = {"id": self._next_id, "subject": subject,
                "status": "pending", "blockedBy": [],
                "owner": ""}             # 新規タスクはオーナーなしで開始
        self._save(task)                 # 即座にディスクに永続化
        self._next_id += 1
...
def _clear_dependency(self, completed_id):
    for f in self.dir.glob("task_*.json"):  # 全タスクをスキャン
        task = json.loads(f.read_text())
        if completed_id in task.get("blockedBy", []):
            task["blockedBy"].remove(completed_id)  # 依存を解除
            self._save(task)
```
...
def update(self, task_id, status=None,
    ...
    if status:
        task["status"] = status
        if status == "completed":           # 完了をグラフに伝播
            self._clear_dependency(task_id)
    if add_blocked_by:                      # 依存エッジを追加
        task["blockedBy"] = list(set(task["blockedBy"] + add_blocked_by))
    if remove_blocked_by:                   # 依存エッジを削除
...
TOOL_HANDLERS = {
    ...
    "task_list":   lambda **kw: TASKS.list_all(),  # LLMがグラフ全体を参照できる
